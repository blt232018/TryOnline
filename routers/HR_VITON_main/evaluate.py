...
def get_opt():
    ...
    parser.add_argument('--ground_truth_dir',
                        default='./data/zalando-hd-resize/test/image')
...
def Evaluation(opt, pred_list, gt_list):
    ...
    inception_model = inception_v3(
        pretrained=True, transform_input=False).type(torch.cuda.FloatTensor)
    ...
    with torch.no_grad():
        ...
        for i, img_pred in enumerate(pred_list):
            ...
            avg_ssim += ssim(gt_np, pred_np, data_range=255,
            ...
            lpips_list.append((img_pred, model.forward(
                gt_img_LPIPS, pred_img_LPIPS).item()))
            avg_distance += lpips_list[-1][1]
            # Calculate Inception model prediction
            pred_img_IS = T3(pred_img).unsqueeze(0).cuda()
            preds[i] = F.softmax(inception_model(
                pred_img_IS)).data.cpu().numpy()
        ...
        for k in range(splits):
            part = preds[k * (len(gt_list) // splits): (k+1)
                         * (len(gt_list) // splits), :]
...
def main():
    ...
    avg_ssim, avg_mse, avg_distance, IS_mean, IS_std = Evaluation(
        opt, pred_list, gt_list)
    print("SSIM : %f / MSE : %f / LPIPS : %f" %
          (avg_ssim, avg_mse, avg_distance))
