...
from cp_dataset import CPDataLoader, CPDataset, CPDatasetTest
from networks import (ConditionGenerator, GANLoss, VGGLoss, define_D,
                      load_checkpoint)
...
def train(opt, train_loader, test_loader, val_loader, board, tocg, D):
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step)):
        ...
        cm_paired = torch.FloatTensor(
            (cm_paired.detach().cpu().numpy() > 0.5).astype(np.float)).cuda()
        # input2
