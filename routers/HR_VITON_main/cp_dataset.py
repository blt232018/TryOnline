...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in ['paired']:
            ...
            c[key] = Image.open(
                osp.join(self.data_path, 'cloth', c_name[key])).convert('RGB')
            c[key] = transforms.Resize(
                self.fine_width, interpolation=2)(c[key])
            cm[key] = Image.open(
                osp.join(self.data_path, 'cloth-mask', c_name[key]))
            cm[key] = transforms.Resize(
                self.fine_width, interpolation=0)(cm[key])
        ...
        # load parsing image
        parse_name = im_name.replace(
            'image', 'image-parse-v3').replace('.jpg', '.png')
        ...
        image_parse_agnostic = Image.open(osp.join(self.data_path, parse_name.replace(
        ...
        # load pose points
        pose_name = im_name.replace(
            'image', 'openpose_img').replace('.jpg', '_rendered.png')
...
class CPDatasetTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_parse = Image.open(
            osp.join(self.data_path, 'image-parse-v3', parse_name))
        im_parse = transforms.Resize(
            self.fine_width, interpolation=0)(im_parse)
        ...
        pose_map = Image.open(
            osp.join(self.data_path, 'openpose_img', pose_name))
        pose_map = transforms.Resize(
            self.fine_width, interpolation=2)(pose_map)
        pose_map = self.transform(pose_map)  # [-1,1]
